...
class ApiClient {
    ...
    constructor(basePath = '/') {
        ...
        this.axiosInstance = axios.create({
            ...
            withCredentials: this.enableCookies
        });
        ...
        this.axiosInstance.interceptors.request.use(
            (config) => {
                ...
                // 防止缓存
                if (config.method === 'get' && this.cache === false) {
    ...
    * @param {Object} requestConfig The request config object.
                ...
                case 'bearer':
                    if (auth.accessToken) {
                        var localVarBearerToken = typeof auth.accessToken === 'function'
                          ? auth.accessToken()
                          : auth.accessToken;
                        requestConfig.headers['Authorization'] = 'Bearer ' + localVarBearerToken;
                    }
                    break;
                case 'apiKey':
                    if (auth.apiKey) {
    ...
    */
    deserialize(response, returnType) {
        ...
        if (data == null || (typeof data === 'object' && typeof data.length === 'undefined' && !Object.keys(data).length)) {
            // If no data, use the status text
    ...
    */
    callApi(path, httpMethod, pathParams,
        ...
        var requestConfig = {
            url: url,
            method: httpMethod,
            headers: {...this.defaultHeaders, ...this.normalizeParams(headerParams)}
        };
        ...
        requestConfig.params = this.normalizeParams(queryParams);

        // 设置 Accept 头
        var accept = this.jsonPreferredMime(accepts);
        if (accept) {
            requestConfig.headers['Accept'] = accept;
        }
        ...
        if (contentType === 'application/x-www-form-urlencoded') {
            ...
            requestConfig.data = urlSearchParams.toString();
            ...
            // 使用formData时axios会自动设置合适的Content-Type，所以删除之前设置的
        ...
        // 应用认证信息
        requestConfig = this.applyAuthToRequest(requestConfig, authNames);

        // 发送请求
        return this.axiosInstance(requestConfig)
            .then(response => {
                if (callback) {
                    callback(null, this.deserialize(response, returnType), response);
                }
